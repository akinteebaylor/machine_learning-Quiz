...
class_details = {i: j for i, j in Animal_dict.values()} #dictionary comprehnsion
...
class_numb = [(Animal(i)).class_number for i in animals]#list comprehension
...
predicted_class_list = [Animal(animals[i]).return_class_type(i) for i in predicted]#list comprehension
#Create a dataframe that has the animal name and the predicted class_type
prediction = pd.Series(predicted_class_list, name="prediction") #converts the predicted class_type to a panda series
